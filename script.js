// ----------------------
// UTILIDADES MEJORADAS (optimizaciones y correcciones)
// ----------------------
...
// Dixon-Coles mejorado con correlación rho dinámica y eliminada tau no utilizada
function dixonColesAdjustment(lambdaH, lambdaA, h, a) {
  const avgLambda = (lambdaH + lambdaA) / 2;
  const rho = -0.15 * Math.min(lambdaH, lambdaA) / Math.max(avgLambda, 1);
...
function normalizeTeam(raw) {
  ...
  const r = {};
  r.name = raw.name.trim(); // Trim para limpieza
  ...
  // Solo retornar equipos con datos válidos y al menos 1 partido
  return r.name && r.pj > 0 ? r : null;
}

// ----------------------
// FETCH EQUIPOS Y CALENDARIO (mejorado con timeout y mejor error handling)
// ----------------------
async function fetchTeams() {
  ...
  try {
    // Añadir timeout para fetch (5 segundos)
    ...
    const errorMsg = `<div class="error"><strong>Error:</strong> No se pudieron cargar los datos de la API. Verifica la conexión o el endpoint. Detalle: ${err.message}</div>`;
...
// ----------------------
// RENDERIZAR PARTIDOS (añadido sorting por liga/hora si es necesario)
// ----------------------
function renderMatches() {
  ...
  // Sort por liga y luego hora (asumiendo hora es string comparable)
  const sortedMatches = [...calendarData.partidosFuturos].sort((a, b) => {
...
// ----------------------
// INICIALIZACIÓN (añadido chequeo de elementos DOM)
// ----------------------
async function init() {
...
// ----------------------
// BÚSQUEDA Y LLENADO DE EQUIPO (mejorado con fallback si no hay datos específicos)
// ----------------------
...
function fillTeamData(teamName, leagueCode, type) {
  ...
  // Mejora: Usa datos específicos de local/visitante si disponibles, sino fallback a general
...
// ----------------------
// CÁLCULO PRINCIPAL MEJORADO (optimizado loop, rho dinámico, maxGoals reducido a 10 para eficiencia)
// ----------------------
function calculateAll() {
  ...
  try {
    // Calcular promedios de la liga (mejorado con validación y fallback)
    ...
    const avgGh = totalGames > 0 ? totalGfHome / totalGames : 1.3; // Fallback más realista (promedio global fútbol)
    const avgGa = totalGames > 0 ? totalGaHome / totalGames : 1.1;

    // Lambdas mejoradas - considera fuerza del oponente y forma
    const homeAdvantage = 1.2; // Ajustado a valor más conservador basado en datos históricos
    ...
    // Factor de forma basado en win rate (suavizado)
    const homeForm = tH.pjHome > 0 ? Math.min(1.4, Math.max(0.8, (tH.winsHome / tH.pjHome) * 1.2 + 0.4)) : 1.0;
    const awayForm = tA.pjAway > 0 ? Math.min(1.4, Math.max(0.8, (tA.winsAway / tA.pjAway) * 1.2 + 0.4)) : 1.0;

    const lambdaH = Math.max(0.2, Math.min(4.5, attackH * defenseA * avgGh * homeAdvantage * homeForm));
    const lambdaA = Math.max(0.2, Math.min(4.5, attackA * defenseH * avgGa * awayForm));

    // Método 1: Poisson estándar (optimizado con precomputo de probs)
    let pHomeP = 0, pDrawP = 0, pAwayP = 0, pBTTSP = 0, pO25P = 0;
    const maxGoals = 10; // Reducido para eficiencia, cubre >99.9% de masa probabilística
    ...
    // Método 2: Dixon-Coles mejorado (sin tau, rho dinámico)
    ...
    const dcWeight = 0.75; // Aumentado a 75% Dixon-Coles para mejor precisión en scores bajos
    ...
    $('dixonColesFactor').textContent = formatDec(rho); // Mostrar rho calculado
    ...
    $('details').innerHTML = `${warning}Goles esperados: ${expectedGoals} (${goalsInfo}). Basado en Dixon-Coles con correlación ajustada (rho: ${formatDec(rho)}) y factor de forma.`;
...
// Análisis rápido sin interfaz (para uso programático, optimizado)
function quickAnalysis(homeAttack, homeDefense, homeWinRate, awayAttack, awayDefense, awayWinRate) {
  try {
    ...
    const avgGh = 1.3, avgGa = 1.1, homeAdvantage = 1.2;
    ...
    const homeForm = Math.min(1.4, Math.max(0.8, homeWinRate / 100 * 1.2 + 0.4));
    const awayForm = Math.min(1.4, Math.max(0.8, awayWinRate / 100 * 1.2 + 0.4));
...
// Función para exportar resultados actuales (mejorada con JSON download)
function exportCurrentResults() {
  const results = {
    ...
    parameters: {
      ...
      dixonColesRho: $('dixonColesFactor')?.textContent || '-0.15'
    },
