...
// ----------------------
// FETCH EQUIPOS (MODIFICADA PARA CALENDARIO)
// ----------------------
async function fetchTeams() {
    ...
    try {
        ...
        if (calendarEventsDiv) {
            calendarEventsDiv.innerHTML = `<div class="calendar-error">Error al cargar el calendario.</div>`;
        }
        if (leagueSelect) leagueSelect.innerHTML = '<option value="">Error al cargar ligas</option>';
        return { ligas: {}, calendario: {} };
    }
}

// Nuevo: Formato de fecha para "Partidos para HOY"
...
async function init() {
    ...
    const calendarDateEl = $('calendarDate');
    if (calendarDateEl) calendarDateEl.textContent = getTodayDateFormatted();
    ...
    Object.keys(teamsByLeague.ligas || {}).sort().forEach(code => {
    ...
    const floatingCalendarContainer = $('floatingCalendarContainer');
    const toggleCalendarBtn = $('toggleCalendarBtn');
    const closeCalendarBtn = $('closeCalendar');

    if (floatingCalendarContainer && toggleCalendarBtn && closeCalendarBtn) {
        toggleCalendarBtn.addEventListener('click', () => {
            floatingCalendarContainer.classList.toggle('show');
        });

        closeCalendarBtn.addEventListener('click', () => {
            floatingCalendarContainer.classList.remove('show');
        });
    }
...
// ----------------------
// NUEVO: FUNCIONES PARA EL CALENDARIO FLOTANTE
// ----------------------
function loadCalendarEvents() {
    ...
    for (const leagueName in calendarData) {
        calendarData[leagueName].forEach(event => {
            if (event.fecha === todayDateString) {
                allTodayEvents.push(event);
            }
        });
    }

    allTodayEvents.sort((a, b) => a.hora.localeCompare(b.hora));
    ...
    if (allTodayEvents.length === 0) {
        calendarEventsDiv.innerHTML = '<div class="calendar-no-events">No hay partidos programados para hoy.</div>';
        return;
    }

    for (let i = 0; i < allTodayEvents.length; i += 2) {
        const globe = document.createElement('div');
        globe.classList.add('calendar-globe');

        const event1 = allTodayEvents[i];
        globe.innerHTML += `
            <div class="calendar-event">
                <strong>${event1.local} vs ${event1.visitante}</strong>
                <span>${event1.hora} - ${event1.estadio} (${event1.liga})</span>
            </div>
        `;

        if (allTodayEvents[i + 1]) {
            const event2 = allTodayEvents[i + 1];
            globe.innerHTML += `
                <div class="calendar-event">
                    <strong>${event2.local} vs ${event2.visitante}</strong>
                    <span>${event2.hora} - ${event2.estadio} (${event2.liga})</span>
                </div>
            `;
        }
        calendarEventsDiv.appendChild(globe);
    }
}
...
function onLeagueChange() {
    ...
    if (!code || !teamsByLeague.ligas[code] || teamsByLeague.ligas[code].length === 0) {
        clearTeamData('Home');
        clearTeamData('Away');
        updateCalcButton();
    ...
    teamsByLeague.ligas[code].forEach(t => {
...
function findTeam(leagueCode, teamName) {
    if (!teamsByLeague.ligas[leagueCode]) return null;
    return teamsByLeague.ligas[leagueCode].find(t => t.name === teamName) || null;
}
...
function calculateAll() {
    ...
    const teams = teamsByLeague.ligas[league];
    let totalGames = 0;
    let totalGfHome = 0;
    let totalGaHome = 0;
    teams.forEach(t => {
