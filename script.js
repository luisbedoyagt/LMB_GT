...
// ----------------------
const WEBAPP_URL = "https://script.google.com/macros/s/AKfycbxQeinF-7q115Z2Hx9V31tFkVc7QnxmcQOTakbFshlL-CnWgrIW31cEwQLttCz4x7-MOQ/exec";
let teamsByLeague = {};
let calendarByLeague = {};
...
// ----------------------
// FETCH EQUIPOS y CALENDARIO
// ----------------------
async function fetchTeamsAndCalendar() {
...
    calendarByLeague = data.calendario || {};
    localStorage.setItem('teamsByLeague', JSON.stringify(normalizedTeams));
    localStorage.setItem('calendarByLeague', JSON.stringify(calendarByLeague));
    return { teams: normalizedTeams, calendar: calendarByLeague };
  } catch (err) {
    console.error('Error en fetchTeamsAndCalendar:', err);
...
  const { teams, calendar } = await fetchTeamsAndCalendar();
  teamsByLeague = teams;
  calendarByLeague = calendar;
...
  // Actualizar el contenido del calendario cada vez que se abre
  const currentLeague = $('leagueSelect').value;
  if (currentLeague) {
    updateCalendarContent(currentLeague);
  } else {
    $('calendarContent').innerHTML = '<p class="no-matches-msg">Selecciona una liga para ver el calendario.</p>';
  }
}

function updateCalendarContent(leagueCode) {
  const calendarContent = $('calendarContent');
  const leagueName = leagueNames[leagueCode] || leagueCode;
  const matches = calendarByLeague[leagueName] || [];
...
      html += `<div class="calendar-league-title">${date}</div>`;
...
    }
  } else {
    html = '<p class="no-matches-msg">No se encontraron partidos próximos para esta liga.</p>';
  }
